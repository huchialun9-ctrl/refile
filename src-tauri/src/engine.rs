...
use crate::transfer::{
    connect_data, receive_file_with_session, send_file_with_session, TransferServer, TransferStats,
};
use crate::types::*;
use anyhow::Result;
...
/// Stores locally-computed hashes during transfer verification
type VerifyingHashes = Arc<Mutex<HashMap<String, String>>>;
...
pub struct TransferEngine {
    ...
    verifying_hashes: VerifyingHashes,
}
...
impl TransferEngine {
    pub fn new(device_id: String, device_name: String) -> Self {
        Self {
            ...
            verifying_hashes: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    pub async fn start(
        ...
        *self.transfer_server.lock().await = Some(Arc::new(srv));
        ...
        let verifying = self.verifying_hashes.clone();
            ...
            while let Some(incoming) = rx.recv().await {
                ...
                match &msg {
                    ...
                    ControlMessage::Complete { session_id, hash } => {
                        let local_hash = {
                            let mut v = verifying.lock().await;
                            v.remove(session_id)
                        };
                        let mut t = transfers_clone.lock().await;
                        if let Some(session) = t.get_mut(session_id) {
                            if let Some(ref local) = local_hash {
                                if local == hash {
                                    session.status = TransferStatus::Completed;
                                } else {
                                    session.status = TransferStatus::Failed(format!("Hash mismatch: expected {hash}, got {local}"));
                                }
                            } else {
                                session.status = TransferStatus::Completed;
                            }
    ...
    async fn do_send(
        ...
        let hash = send_file_with_session(&mut data_stream, &session_id, &path, &stats.as_ref()).await?;
    ...
    async fn do_receive(
        ...
        if received_session_id != session_id {
            anyhow::bail!("Session ID mismatch: expected {session_id}, got {received_session_id}");
        }

        {
            let mut t = transfers.lock().await;
            if let Some(s) = t.get_mut(&session_id) {
                s.status = TransferStatus::Verifying;
                s.progress = 1.0;
                s.hash.clone_from(&hash);
            }
        }
        {
